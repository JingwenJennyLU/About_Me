'''
MACS 30121: PA 6 - Avian Biodiversity Treemap
...
class Rectangle:
    ...
    def compute_rec_distort(self):
        '''
        calculate the distortion of the rectangle
        '''
    ...
    def set_label(self, label):
        '''
        set the label of the rectangle
        '''
    ...
    def set_color(self, color):
        '''
        set the color code of the rectangle
        '''
...
def compute_rectangles(t, bounding_rec_width=1.0, bounding_rec_height=1.0):
    ...
    row_layout = compute_rec_recursion(t, bounding_rec)
    lst_of_rec = [rec for rec, tr in row_layout]
    return lst_of_rec


def compute_rec_recursion(t, bounding_rec):
    '''
    Computes the rectangles for drawing a treemap of the provided tree,

    Inputs:
        t: (Tree) a tree
        bounding_rec: a rectangle

    Returns: a list of Rectangle objects and its corresponding tree
    '''

    if type(t) is not list:
        child_lst = t.children
        total_num = t.num_children()
    else:
        child_lst = t
        total_num = len(t)

    sorted_child_lst = sorted_trees(child_lst)
    total_sum = sum(trees.value for trees in sorted_child_lst)
    ...
    for k in range(1, total_num + 1):  
        row_layout, leftover_rec = compute_row(bounding_rec,
                                               sorted_child_lst[:k],
                                               total_sum)
        k_distortion = compute_row_distortion(row_layout)

        if k_distortion <= best_distortion:
            best_distortion = k_distortion
            best_row_layout = row_layout
            best_leftover_rec = leftover_rec
            best_k = k
        else:
            break

    final_row_layout = []
    for rec, tr in best_row_layout:
        if tr.num_children() == 0:
            rec.set_color(tr.path)
            final_row_layout.extend([(rec, tr)])
        else:
            final_row_layout.extend(compute_rec_recursion(tr, rec))
    if best_k < total_num:
        final_row_layout.extend(compute_rec_recursion(
            sorted_child_lst[best_k:],
            best_leftover_rec))

    return final_row_layout


def compute_row_distortion(row_layout):
    '''
    Compute the distortion of the row and add label to rectangle

    Input: row_layout: list of pairs (rec, t), where rec is a Rectangle, and t 
        is the Tree that
        Rectangle corresponds to
    Return: distortion of the row
    '''

    distortion = 0
    for rec, t in row_layout:
        rec.set_label(t.key)
        rec.set_color(t.path)
        rec_dis = rec.compute_rec_distort()
        distortion = max(distortion, rec_dis)
    return distortion
...
def compute_internal_values(t):
    ...
    if t.num_children() == 0:
        ...
        for child in t.children:
            total += compute_internal_values(child)
        t.value = total
        return t.value
